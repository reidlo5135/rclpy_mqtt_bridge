...
class dynamic_bridge(Node):
    ...
    def __lookup_object__(self, object_path: str) -> Any:
        self.get_logger().info(
            "{} lookup object path : {}".format(self.__rclpy_flags__, object_path)
        )
        ...
        self.get_logger().info(
            "{} lookup object module_name : {}".format(
                self.__rclpy_flags__, module_name
            )
        )
    ...
    def __parse_rcl_topic_type__(self, topic_type: str) -> Any:
        ...
        self.get_logger().info(
            "{} parsed topic type : [{}]".format(
                self.__rclpy_flags__, parsed_topic_type
            )
        )
    ...
    def __publisher_to_subscription__(self, topic_name: str, topic_type: str) -> None:
        ...
        if publishers > 0:
            self.get_logger().info(
                "{} [{}] is a publisher".format(self.__rclpy_flags__, topic_name)
            )

            is_ignored_topic_name: bool = (topic_name == "/parameter_events") or (
                topic_name == "/rosout"
            )

            if is_ignored_topic_name:
                self.get_logger().warn(
                    "{} ignoring [{}] publisher".format(
                        self.__rclpy_flags__, topic_name
                    )
                )
                return
            elif topic_name in self.__established_rcl_subscriptions_list__:
                self.get_logger().warn(
                    "{} [{}] pub to sub connection is already established... ignoring".format(
                        self.__rclpy_flags__, topic_name
                    )
                )
                return
            ...
            def rcl_subscription_callback(ros_message) -> None:
            ...
            self.create_subscription(
                parsed_topic_type, topic_name, rcl_subscription_callback, 10
            )

            if topic_name not in self.__established_rcl_subscriptions_list__:
                self.get_logger().info(
                    "===== {} [{}] pub to sub connection established =====".format(
                        self.__rclpy_flags__, topic_name
                    )
                )
            ...
            self.get_logger().info(
                "{} [{}] is not a publisher".format(self.__rclpy_flags__, topic_name)
            )
    ...
    def __subscription_to_publisher__(self, topic_name: str, topic_type: str) -> None:
        ...
        if subscriptions > 0:
            self.get_logger().info(
                "{} [{}] is a subscription".format(self.__rclpy_flags__, topic_name)
            )

            if topic_name in self.__established_rcl_publishers_list__:
                self.get_logger().warn(
                    "{} [{}] sub to pub connection is already established... ignoring".format(
                        self.__rclpy_flags__, topic_name
                    )
                )
                return
            ...
            rcl_publisher: Publisher = self.create_publisher(
                parsed_rcl_topic_type, topic_name, 10
            )

            def mqtt_subscription_callback(
                client: mqtt.Client, user_data: Dict, mqtt_message: mqtt.MQTTMessage
            ):
                mqtt_topic: str = mqtt_message.topic
                mqtt_decoded_payload: str = mqtt_message.payload.decode()

                self.get_logger().info(
                    "{} MQTT received message [{}] from [{}]".format(
                        self.__rclpy_flags__, mqtt_decoded_payload, mqtt_message.topic
                    )
                )
                ...
                if is_rcl_mqtt_topic_equals == False:
                    self.get_logger().error(
                        "{} topic RCL & MQTT topics is not matching each other"
                    )
                    return
                else:
                    rcl_deserialized_message: Any = json.loads(mqtt_message.payload)
                    self.get_logger().info(
                        "{} deserialized message : [{}]".format(
                            self.__rclpy_flags__, rcl_deserialized_message
                        )
                    )
                    rcl_message_type: Any = self.__lookup_object__(topic_type)

                    created_rcl_messages: Any = message_conversion.populate_instance(
                        rcl_deserialized_message, rcl_message_type()
                    )
                    rcl_publisher.publish(created_rcl_messages)

            if topic_name not in self.__established_rcl_publishers_list__:
                self.get_logger().info(
                    "===== {} [{}] sub to pub connection established =====".format(
                        self.__rclpy_flags__, topic_name
                    )
                )
                ...
                self.__mqtt_manager__.client.message_callback_add(
                    topic_name, mqtt_subscription_callback
                )
            else:
                return
        else:
            self.get_logger().info(
                "{} [{}] is not a subscription".format(self.__rclpy_flags__, topic_name)
            )
    ...
    def __service_request__(self, service_name: str, service_type: str) -> None:
        ...
        if is_ignoring_required_service_server:
            self.get_logger().warn(
                "{} ignoring [{}] service client...".format(
                    self.__rclpy_flags__, service_name
                )
            )
            return
        elif service_name in self.__established_rcl_service_client_list__:
            self.get_logger().warn(
                "{} [{}] service client connection is already established... ignoring".format(
                    self.__rclpy_flags__, service_name
                )
            )
            return
        ...
        rcl_service_client: Client = self.create_client(
            parsed_rcl_service_type, service_name
        )
        ...
        def mqtt_subscription_callback(
            ...
            self.get_logger().info(
                "{} MQTT service request received message [{}] from [{}]".format(
                    self.__rclpy_flags__, mqtt_decoded_payload, mqtt_message.topic
                )
            )

            is_rcl_mqtt_topic_equals: bool = mqtt_topic == rcl_service_client.srv_name

            if is_rcl_mqtt_topic_equals == False:
                self.get_logger().error(
                    "{} service request RCL & MQTT topics is not matching each other"
                )
                return
            else:
                rcl_deserialized_request_message: Any = json.loads(mqtt_message.payload)
                self.get_logger().info(
                    "{} service request deserialized message : [{}]".format(
                        self.__rclpy_flags__, rcl_deserialized_request_message
                    )
                )

                rcl_message_type: Any = self.__lookup_object__(service_type)
                created_rcl_messages: Any = message_conversion.populate_instance(
                    rcl_deserialized_request_message, rcl_message_type()
                )

                service_server_waiting_time: float = 1.0
                is_service_server_ready: bool = rcl_service_client.wait_for_service(
                    service_server_waiting_time
                )

                if not is_service_server_ready:
                    self.get_logger().error(
                        "{} [{}] service server is not ready yet...".format(
                            self.__rclpy_flags__, service_name
                        )
                    )
                    ...
                    self.__mqtt_manager__.publish(
                        mqtt_response_topic, mqtt_service_server_error_message
                    )
                    return
                else:
                    self.get_logger().info(
                        "{} calling to [{}] service server".format(
                            self.__rclpy_flags__, service_name
                        )
                    )

                    service_future: Future = rcl_service_client.call_async(
                        created_rcl_messages
                    )
                    service_call_async_result: Any = service_future.result()

                    rcl_serialized_response_message: str = json.dumps(
                        message_conversion.extract_values(service_call_async_result)
                    )
            ...
            if service_name not in self.__established_rcl_publishers_list__:
                self.get_logger().info(
                    "===== {} [{}] service client connection established =====".format(
                        self.__rclpy_flags__, service_name
                    )
                )
    ...
    def __action_request__(self, action_name: str, action_type: str) -> None:
        if action_name in self.__established_rcl_action_client_list__:
            self.get_logger().warn(
                "{} [{}] action client connection is already established... ignoring".format(
                    self.__rclpy_flags__, action_name
                )
            )
            return

        parsed_rcl_action_type: Any = self.__parse_rcl_topic_type__(action_type)
        rcl_action_client: ActionClient = ActionClient(
            self, parsed_rcl_action_type, action_name
        )
        ...
        def mqtt_subscription_callback(
            ...
            self.get_logger().info(
                "{} MQTT action send goal received message [{}] from [{}]".format(
                    self.__rclpy_flags__, mqtt_decoded_payload, mqtt_message.topic
                )
            )

            is_rcl_mqtt_topic_equals: bool = (
                mqtt_topic == rcl_action_client._action_name
            )

            if is_rcl_mqtt_topic_equals == False:
                self.get_logger().error(
                    "{} action send goal RCL & MQTT topics is not matching each other"
                )
                return
            else:
                rcl_deserialized_request_message: Any = json.loads(mqtt_message.payload)
                self.get_logger().info(
                    "{} action send goal deserialized message : [{}]".format(
                        self.__rclpy_flags__, rcl_deserialized_request_message
                    )
                )
                ...
                is_action_server_ready: Any = rcl_action_client.wait_for_server(
                    action_server_waiting_time
                )

                if not is_action_server_ready:
                    self.get_logger().error(
                        "{} [{}] action server is not ready yet...".format(
                            self.__rclpy_flags__, action_name
                        )
                    )
                    ...
                    self.__mqtt_manager__.publish(
                        mqtt_response_topic, mqtt_action_server_error_message
                    )
                    return
                else:
                    action_future: Future = rcl_action_client.send_goal_async(
                        created_rcl_messages
                    )
                    action_send_goal_result: Any = action_future.result()

                    rcl_serialized_response_message: str = json.dumps(
                        message_conversion.extract_values(action_send_goal_result)
                    )
            ...
            if action_name not in self.__established_rcl_publishers_list__:
                self.get_logger().info(
                    "===== {} [{}] action client connection established =====".format(
                        self.__rclpy_flags__, action_name
                    )
                )
                self.__established_rcl_action_client_list__.append(action_name)
                self.__mqtt_manager__.subscribe(action_name)
                self.__mqtt_manager__.client.message_callback_add(
                    action_name, mqtt_subscription_callback
                )
            else:
                return
    ...
    def __bridge__(self) -> None:
        ...
        for service_name, service_type_list in service_and_types:
            for service_type in service_type_list:
                self.get_logger().info(
                    "{} service : [{}], type : [{}]".format(
                        self.__rclpy_flags__, service_name, service_type
                    )
                )
        ...
        for action_name, action_type_list in action_and_types:
            for action_type in action_type_list:
                self.get_logger().info(
                    "{} action : [{}], type : [{}]".format(
                        self.__rclpy_flags__, action_name, action_type
                    )
                )
